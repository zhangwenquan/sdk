...
{
    public class GivenThatWeWantToBuildACppCliProjectWithTransivitDeps : SdkTest
    {
        public GivenThatWeWantToBuildACppCliProjectWithTransivitDeps(ITestOutputHelper log) : base(log)
        {
        ...
        public void It_can_generate_correct_depsJson_file()
        {
            ...
            string depsJsonContent = File.ReadAllText(Path.Combine(testAsset.TestRoot, "NETCoreCppCliTest", "Debug", "NETCoreCppCliTest.deps.json"));
        ...
        public void It_can_generate_all_runtimeconfig_files_to_output_folder()
        {
            ...
            outputDirectory.Should().HaveFiles(new[] { "NETCoreCppCliTest.runtimeconfig.json" , "NETCoreCppCliTestB.runtimeconfig.json", "NETCoreCppCliTestC.runtimeconfig.json" });
        }
        ...
        public void It_can_generate_all_depsjson_files_to_output_folder()
        {
            TestAsset testAsset = _buildAsset.Value;
            var outputDirectory = new DirectoryInfo(Path.Combine(testAsset.TestRoot, "NETCoreCppCliTest", "Debug"));
            outputDirectory.Should().HaveFiles(new[] { "NETCoreCppCliTest.deps.json", "NETCoreCppCliTestB.deps.json", "NETCoreCppCliTestC.deps.json" });
        }
